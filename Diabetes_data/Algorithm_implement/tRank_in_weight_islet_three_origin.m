...
PP_weight = PP.data(:,1);
% Beta
Be =  importdata('Selected_Beta.csv');
...
Node=unique([Node_alp',Node_PP',Node_Be']);
...
for(i = 1:length(Alpmcol))
    if(Alpmcol(i) >0)
        ...
        Alpt(j,i) =Alpm(j,i)/Alpmcol(i);%Alpmw(j,i)/Alpmwcol(i);
        end
    else
...
save alp_beta_PP_origin_new.mat D_PR D_Rank loc Node Rank R_PR R_Rank '-v6'

...
%%  data import
% Ast
% Ast = importdata('Plaswith_imputaion_dmi.csv');
% Ast = importdata('Hepat_without_imputaion_dmi_1.csv');
% Ast = importdata('Plaswithout_imputaion_dmi.csv');
...
Node=unique(Node1);
% fid=fopen('all_node.txt','w');
% for jj=1:length(Node1)
%     fprintf(fid,'%s\r\n',Node1{jj});   %
% %     fprintf(fid,'%.4\t',A(jj)); 
% end
% fclose(fid);
...
% end
% fclose(fid);
% Ad2= importdata('kegg_disease_genes.txt');
% Ad2=Ad2(2:end);
% loc2=[];j = 1;
% for(ii = 1:length(Ad2))
%      z2=strcmp(Ad2{ii},Node);
...
for(i = 1:length(Acol))
    if(Acol(i) >0)
        for(j=1:length(Acol))
        At(j,i) =A1w(j,i)/Awcol(i);%*A1(j,i)/Acol(i);%
        end
    else
        At(:,i) =1/length(Node);
    end
...
save alp_weigthed_new.mat D_PR D_Rank loc Node Rank R_PR R_Rank '-v6'
%% 排名靠前的node
% loo =find(Rank<=30);
% Former_node=[];
% for(j =1:length(loo))
